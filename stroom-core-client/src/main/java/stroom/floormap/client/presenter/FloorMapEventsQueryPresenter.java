...
public class FloorMapEventsQueryPresenter
        extends DocPresenter<FloorMapQueryView, FloorMapDoc>
    ...
    @Inject
    public FloorMapEventsQueryPresenter(final EventBus eventBus,
    ...
    protected void onBind() {
        ...
        registerHandler(queryEditPresenter.getQueryResultPresenter().getTablePresenter().addUpdateHandler(e -> {
            ...
            if (tableResult != null) {
                ...
                FloorMapDataEvent.fire(FloorMapEventsQueryPresenter.this, objects);
            }
        }));

        // Call onChange when the query settings are modified so the document registers as dirty.
    ...
    protected void onRead(final DocRef docRef, final FloorMapDoc document, final boolean readOnly) {
        this.currentEntityColumn = document.getEntityIdColumn();
        this.currentLocationColumn = document.getLocationIdColumn();

        getView().setEntityIdColumn(currentEntityColumn);
        getView().setLocationIdColumn(currentLocationColumn);
        getView().setColumnMappingsVisible(true);

        // Populate the inner query editor.
        queryEditPresenter.setQuery(docRef, document.getEventsQuery(), false);
        queryEditPresenter.setTimeRange(document.getEventsQueryTimeRange());
        queryEditPresenter.read(document.getEventsQueryTablePreferences());
