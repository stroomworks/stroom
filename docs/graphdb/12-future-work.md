...
| 5a | **Sweep the property-value lookup** | The remaining unbounded table: one entry per distinct property value above the inline length tier, ever seen | Easy–Medium | Medium — needs `GraphPropertyIndex.insert` to report which lookup tier and entry it used, so the rebuild can record it. Sweeping without that deletes entries live anchors reference, and every affected query then silently returns nothing |
