...
| 5 | **Retention for the property index** — include it and the property-key table in the sweep | The index grows without bound regardless of retention. **Harder than it looks:** the table has no `validFrom`, so it cannot be aged; and its values cannot be decoded back out of a stored key, so it cannot be pruned row by row either. The only correct approach is to clear and re-derive it from the surviving node versions, which also means the property-key and property-value lookups may only be swept when that rebuild has run — sweeping them without recording usage first deletes every entry and makes all property-anchored queries miss | Medium | **High** — an incorrect rebuild destroys the index rather than merely wasting space |
