...
*Facts verified on 2026-07-29 against branch `sw-query-optimiser`. Each row names the source constant so a
maintainer can re-verify by grep — and `TestDocumentationReferences` now does it on every build, comparing both
the constant and the value printed beside it ([14-testing.md](14-testing.md#documented-references)).*
...
| Starter queries generated by Discover | `LIMIT 100` | The generated query text | `GraphDiscoveryWidget.WHOLE_GRAPH_LIMIT`, `GraphDiscoveryWidget.LABEL_LIMIT` |
