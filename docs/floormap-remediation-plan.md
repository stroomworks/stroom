...
- **The Events Query tab's role dropdowns offer nothing until the query is run there.** They are populated from the result's columns by `updateColumnSelections`, which only fires on a table update or the searching→idle edge — so on first opening the tab the four dropdowns *display* the document's mapping correctly but cannot be changed. A reader could reasonably conclude the mapping is unset or broken. Verified **not** a data-loss path: `BaseSelectionBox.setValue` stores the value independently of the model and `clear()` only empties the item list, so `write()` reads back what was set and a save does not wipe the mapping. The fix, if wanted, is to seed the lists from the aliases in the query *text* rather than waiting for a result — the query builder already knows them. Reported as the reason test G3 could not be followed as written.
