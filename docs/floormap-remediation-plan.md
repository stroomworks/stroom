...
- ~~**The coordinate location format carried a leading map/building token nothing read**~~ — **DONE**. `"<map>, <x>, <y>"` became `"<x>, <y>"`. The token was early example data that had become *syntax*: no line of code read it, but `parts.length < 3` was what told coordinates apart from a fact key, so deleting it from the data alone would have made every coordinate silently read as a missing fact key. What actually disambiguates is both parts being numeric — `"Desk 12, North"` still reads as a key. **The three-part form is rejected, not reinterpreted**, and deliberately: accepting both would mean guessing at `"1, 120.5, 340"`, where a numeric floor id is indistinguishable from an x — `(120.5, 340)` read as three parts, `(1, 120.5)` read as two, and one of those is silently wrong for somebody. `looksLikeLegacyCoordinates` exists purely so the console can name the retired format instead of blaming the column settings, which is where the reader would otherwise be sent. Breaking change, authorised; the guide carries a migration note and the fixtures carry `migrate-forklift-coords.csv`.

- ~~**Events had three mechanisms for connecting a column name to a meaning**~~ — **DONE 2026-09-07**, and the location split with it. `FloorMapEventRole` + `FloorMapEventColumns` replace `entityIdColumn`, `locationIdColumn` and the hardcoded `equalsIgnoreCase("type")` with one role-to-column mapping, one dropdown per role on the Events Query tab, and a default mapping generated from the same constants as the default query — so the pairing the `Event Type` defect broke cannot come apart again. **Location became two roles**, `LOCATION` for `"x, y"` and `LOCATION_REF` for a fact key, which the write-up had not anticipated: one column carrying both, told apart by shape, made a fact key that looked like two numbers or contained a comma inexpressible, and reported a malformed coordinate as a missing desk. Two roles need no discrimination, so this deleted more code than the mapping added — `looksLikeLegacyCoordinates` and the shape rule are gone. **Both set is contradictory data**: `location` wins, since it needs no lookup, and the clash is reported once per document. `Status`/`Message` deliberately did **not** become roles — nothing reads them. Breaking change, authorised; the events CSV gained a column, and `manifest.json` now carries the landmark times so the protocol stops going stale. Closes `task-floormap-events-column-mapping.md`.

**Small UX items found while testing, 2026-09-07:**

- ~~**Changing an events-column dropdown did not mark the document dirty**~~ — **DONE 2026-09-07**. The save icon stayed disabled, so the mapping could not be saved on its own; the tab wires dirty from `addChangeHandler`, which delegated entirely to the embedded query editor and knew nothing about the dropdowns. Worse than "cannot save": `write()` reads the dropdowns on any save, so the edit persisted **only** as a passenger on an unrelated query-text edit — it worked sometimes, and was otherwise lost on the next tab switch with no dirty marker. Pre-existing: the two string settings this replaced had the same gap, so they were never editable on their own either. The view now exposes a change signal and `addChangeHandler` registers on both sources.


- **The Events Query tab's role dropdowns offer nothing until the query is run there** — written up as `task-floormap-event-column-dropdowns-empty.md`. They are populated only from a query *result*, so on first opening the tab the four dropdowns display the document's mapping correctly but cannot be changed. Verified **not** a data-loss path: `BaseSelectionBox` stores its value independently of the item model, so `write()` reads back what was set and a save does not wipe the mapping. **Two obvious fixes do not work** and are recorded so they are not re-proposed: the query builder knows only the *default* aliases, not a hand-edited query's; and `queryTablePreferences.getColumns()` is populated only when the user has edited the results table's columns. Reported as the reason test G3 could not be followed as written.
- **A role cannot be pointed at a column the query does not select.** The dropdown offers only the result's own columns plus a blank, which is a good property — it means a stale mapping can arise only by editing the *query* after the mapping, never by mis-picking in the UI. Worth knowing because it bounds where that class of fault comes from, and because it made the original G3 unrunnable: the reachable fault is unmapping a role, not misdirecting it.
...
- ~~**F11 · asset servlet**~~ — **out of scope for this branch, 2026-09-04: already raised as a separate issue elsewhere.** Recorded so it is not re-litigated. Analysis worth keeping if it is picked up: the global CSP does **not** mitigate it (`script-src 'self' 'unsafe-eval' 'unsafe-inline'`, deliberately permissive for visualisations, and an uploaded asset is served *from* `'self'`); sandboxing the vis iframe closes little, because `Script` documents already inject arbitrary JS there by design; the genuinely new exposure is a **direct URL rendering uploaded HTML in the app's own origin**, which needs only `Content-Disposition`/`nosniff` or a per-response `default-src 'none'` to close — though `VisFrame.setUrl` exists precisely to serve assets into the frame, so that needs checking against real usage. Severity is bounded: planting needs EDIT, triggering needs VIEW and a session, so it is escalation within an authenticated user base rather than external surface. The proper answer is serving assets from a separate origin.
...
- ~~**The `fetchAll` OpenAPI correction**~~ — **closed 2026-09-10.** Two halves, and both are now
  answered. *Whom to tell:* nobody. `stroom-sqlstore` does not exist on `origin/master`, the
  CHANGELOG has never mentioned it, and its only change entry is the still-pending Floor Map feature
  one — so the API has never shipped and no consumer could have read the claim from a released spec.
  *What to fix:* the committed OpenAPI spec still carried it, which F9 missed. Both files now match
  the annotation verbatim — and, verified by running the generator, exactly what it produces.
  **Why it went stale is the part worth keeping.** The spec *is* generated, by the `resolve` task in
  `stroom-app/build.gradle`, but that hangs off `tasks.build`, not `check`. So the standard
  pre-commit verification never regenerates it, and an `@Operation` edit sits in the source looking
  done while the served description still says the opposite. (There is also a commented-out block
  for an older swagger plugin in the same file, which is easy to mistake for the whole story — I
  did.)
...
### The generated spec was missed — corrected 2026-09-10

Fixing the `@Operation` annotation did not fix what is served, so this finding was reported as done
on 2026-09-04 while the false claim was still in the file callers read. Both spec files now match
the annotation, and running the generator confirms that is exactly what it emits.

**The mechanism, because it will catch the next person too.** The spec is generated by the `resolve`
task in `stroom-app/build.gradle`, which is wired to `tasks.build` — *not* to `check`. The repo's
pre-commit convention is `./gradlew check`, so the generator never runs during ordinary verification
and a corrected annotation looks applied while the committed artefact still contradicts it. The same
file also holds a large commented-out block configuring an *older* swagger plugin, which is easy to
read as "generation is disabled" when it is not.

**A wider consequence, not ours to fix.** Running `resolve` on this branch rewrites about 811 lines,
and every one of them is `/traces/*` — upstream's Traces API, whose spec was not regenerated when
that work landed. `v1` paths that no longer exist are still published and the `v2` ones are missing.
Left alone deliberately: committing it here would mean owning an 811-line diff for someone else's
feature, and the fix belongs with whoever ships Traces.
