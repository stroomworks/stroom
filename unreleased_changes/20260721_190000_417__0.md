* Feature : Allow a StroomQL `join` to take a bracketed Cypher sub-query over a Graph DB as one side, with the schema derived from the sub-query's `RETURN ... AS` columns.
