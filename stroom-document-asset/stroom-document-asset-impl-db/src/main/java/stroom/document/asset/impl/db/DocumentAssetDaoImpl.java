 ...
 *
 */
public class DocumentAssetDaoImpl implements DocumentAssetDao {

    private static final LambdaLogger LOGGER = LambdaLoggerFactory.getLogger(DocumentAssetDaoImpl.class);
    ...
    private final DocumentAssetDbConnProvider connProvider;

    @SuppressWarnings("unused")
    @Inject
    DocumentAssetDaoImpl(final DocumentAssetDbConnProvider connProvider) {
     ...
     */
    private List<DocumentAsset> resultToAssets(final Result<Record3<String, String, Byte>> result) {
        final List<DocumentAsset> assets = new ArrayList<>(result.size());
        for (final Record3<String, String, Byte> record : result) {
            final DocumentAsset asset = new DocumentAsset(record.value1(),
                    record.value2(),
    ...
    public DocumentAssets fetchDraftAssets(final String userUuid,
        ...
        try {
            ...
            final List<DocumentAsset> assets = new ArrayList<>();
            ...
            return new DocumentAssets(ownerDocId, dirty[0], null, assets);

        } catch (final DataAccessException e) {
            LOGGER.error("Error fetching draft document assets for user {}, document {}: {}",
                    userUuid, ownerDocId, e.getMessage(), e);
            throw new IOException("Error fetching document assets: " + e.getMessage(), e);
