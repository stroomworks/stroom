...
public class PipelineFactory {
    ...
    public Pipeline create(final PipelineData pipelineData,
        ...
        for (final PipelineElement element : pipelineData.getAddedElements()) {
            ...
            setPipelineReferences(pipelineElementRegistry, element.getId().id(), element.getType(), elementInstance,
                    pipelineData);

            // Store the instance.
            elementInstances.put(element.getId().id(), elementInstance);
            ...
            linkSets.put(element.getId().id(), linkSet);
    ...
    private void link(final Map<String, Element> elementInstances,
        ...
        // to work with.
        final List<Element> childElements = getChildElements(parentElementId.id(), elementInstances, elementTypeMap,
                linkSets, controller);
    ...
    private Fragment insertRecorder(final ElementId elementId,
        ...
        if (!input) {
            steppingFilterSettings = controller.getRequest().getStepFilterSettings(elementId.id());
        }
