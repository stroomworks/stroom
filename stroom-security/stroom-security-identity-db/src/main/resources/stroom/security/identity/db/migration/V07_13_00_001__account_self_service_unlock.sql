...
DROP PROCEDURE IF EXISTS V07_13_00_001__account_self_service_unlock;

DELIMITER $$

CREATE PROCEDURE V07_13_00_001__account_self_service_unlock ()
BEGIN
    DECLARE object_count integer;
    DECLARE duplicate_count integer;
    DECLARE duplicate_emails text;

    -- Holds the nonce of the most recently issued password reset token for the account. The token
    -- carries the same value as a claim and is only accepted while the two match, so issuing a new
    -- reset token invalidates any earlier one. Cleared when the password is set, so a token cannot be
    -- used twice.
    SELECT COUNT(1)
    INTO object_count
    ...
    AND column_name = 'reset_token_nonce';

    IF object_count = 0 THEN
        ALTER TABLE account ADD COLUMN reset_token_nonce varchar(36) DEFAULT NULL;
    END IF;
...
CALL V07_13_00_001__account_self_service_unlock;
