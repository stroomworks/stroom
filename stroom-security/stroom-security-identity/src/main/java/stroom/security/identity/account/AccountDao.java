...
public interface AccountDao {
     ...
     * consumes the outstanding reset token nonce so the link cannot be used again.
     * <p>
     ...
     * </p>
     *
     * @param expectedResetTokenNonce The nonce the caller believes is outstanding. The password is only
     *                                set if it is still the one held against the account, so that of two
     *                                requests using the same link only the first does anything.
     * @return True if the password was set, false if the account no longer has this nonce outstanding,
     * i.e. the link has already been used or a newer one has been issued.
     */
    boolean unlockAndSetPassword(String userId, String newPassword, String expectedResetTokenNonce);
    ...
    /**
     * Record the nonce of the password reset token just issued for this account, replacing any previous
     * one so that only the most recently issued token is accepted.
     */
    void setResetTokenNonce(String userId, String nonce);

    /**
     * @return The nonce of the most recently issued password reset token for this account, or empty if
     * there is no such account or no reset token is outstanding for it.
     */
    Optional<String> getResetTokenNonce(String userId);
