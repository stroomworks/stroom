...
public class PlanBDocCacheImpl implements PlanBDocCache, Clearable, EntityEvent.Handler {
    ...
    private PlanBDoc create(final String name) {
        return securityContext.asProcessingUserResult(() ->
                documentActionHandlersProvider.get().stream()
                        .filter(handler -> handler instanceof HasFindDocsByName)
                        .map(handler -> {
                            final HasFindDocsByName hasFindDocsByName = (HasFindDocsByName) handler;
                            final List<DocRef> list = hasFindDocsByName.findByName(name);
                            if (list != null && !list.isEmpty()) {
                                final DocRef docRef = list.getFirst();
                                final Object loaded = handler.readDocument(docRef);
                                if (loaded instanceof PlanBDoc planBDoc) {
                                    return planBDoc;
                                }
                            }
                            return null;
                        ...
                        .findFirst()
                        .orElseThrow(() -> new NullPointerException("No state doc can be found for key: " + name))
        );
    }
