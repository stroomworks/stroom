...
public class TerminatedStringKeySerde implements TemporalKeySerde {
    ...
    public TerminatedStringKeySerde(final ByteBuffers byteBuffers,
                                    final TimeSerde timeSerde) {
        this.byteBuffers = byteBuffers;
        this.timeSerde = timeSerde;
        // One byte of the key budget goes to the terminator.
        this.limit = Db.MAX_KEY_LENGTH - timeSerde.getSize() - 1;
        reusableWriteBuffer = ByteBuffer.allocateDirect(Db.MAX_KEY_LENGTH);
    }
    ...
    public TemporalKey read(final Txn<ByteBuffer> txn, final ByteBuffer byteBuffer) {
        ...
        // Slice off the name, dropping the terminator that separates it from the time.
        final ByteBuffer nameSlice = byteBuffer.slice(0,
                byteBuffer.remaining() - timeSerde.getSize() - 1);
    ...
    public <R> R toBufferForGet(final Txn<ByteBuffer> txn,
        ...
        final byte[] bytes = keyBytes(key);
        return byteBuffers.use(bytes.length + 1 + timeSerde.getSize(), byteBuffer -> {
        ...
        for (final byte b : bytes) {
            if (b == TERMINATOR) {
                throw new IllegalArgumentException(
                        "Key contains an embedded null byte, which a terminated string key cannot " +
                        "represent: " + key.getPrefix().getVal());
            }
        }
        return bytes;
    }
